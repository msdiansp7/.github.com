...
nb.add(page2,text="Create account")
...
button.grid(row=2,columnspan=2,padx=100,pady=15)
...
def SignUp():
    ...
    Year=Byear.get()
    Gender=gender.get()
    info=[username,mail,phone,AlterMail,Pass]
    DOB=f"{Date}/{Month}/{Year}"
    with open("Users.csv","a") as f1:
        csv_reader=DictReader(f1)
...
submit_btn=tk.Button(page2,text="Sign up",command=SignUp)
